...
/**
 Port of https://deno.land/x/opine@1.8.0/src/utils/send.ts for INJIN-Z
 */
...
import {
  parseHttpDate,
  parseTokenList,
} from "https://deno.land/x/opine@1.8.0/src/utils/fresh.ts";
...
function collapseLeadingSlashes(str: string) {
  ...
  return i > 1 ? sep + str.substr(i) : str;
}
...
async function sendFile(
  ...
  try {
    ...
    return res.redirect(301, collapseLeadingSlashes(path + sep));
  } catch (err) {
    if (
